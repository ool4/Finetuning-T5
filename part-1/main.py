...
def create_augmented_dataloader(args, dataset):
    ...
    #add 5k random transformed examples to the original training set

    sampled_dataset = dataset['train'].shuffle(seed=42).select(range(5000))

    trans_dataset = sampled_dataset.map(custom_transform, load_from_cache_file=False)

    new_train = datasets.concatenate_datasets([dataset['train'], trans_dataset])

    tokenized_dataset = new_train.map(tokenize_function, batched=True, load_from_cache_file=False)
